...
namespace Pymex.Services
{
    ...
    public class ProductoService : IProductoService
    {
        ...
        public ResponseWithDataDataContract<IEnumerable<ProductoDC>> ListarPorExpresionYCantidad(string expresion, int maxCantidad)
        {
            var response = new ResponseWithDataDataContract<IEnumerable<ProductoDC>>();
            ...
            try
            {
                ...
                {
                    response.Data = (from producto in db.Producto
                                     where producto.Descripcion.Contains(expresion) && producto.Activo
                                     select producto)
                                    .Include(p => p.Almacen)
                                    .Include (p => p.Categoria)
                                    .Take(maxCantidad)
